...
export class AdvisorStep3Component implements OnInit {
	...
	private page_from = 1;
	step3_id : number;
	model = {type_3_diagnosis_id: '',type_12_diagnosis_id: '',type_60_diagnosis_id: '',step3:''};
	...
	private handleResponse(commonResponse:any) {
        if (commonResponse.success) {
            this.step3s = commonResponse.data.data;
            this.total = commonResponse.data.total_items;
            this.number_items =  this.step3s.length;
            this.paginate();
        } else {
            alert(commonResponse.error);
        }
	...
	editStep(step3_id){
		this.step3_id = step3_id;
        let data = {
            'step3_id': step3_id,
        };
        let url = 'advisor/step3/detail';
	...
	private handleResponseDetail(commonResponse:any) {
        if (commonResponse.success) {
			this.model.step3 = commonResponse.data[0];			
        } else {
            alert(commonResponse.error);
        }
    }
	...
	edit(){
		let url = 'advisor/step3/edit';
        let data = {
                    'step3_id' : this.step3_id,
                    'type_3_advice': this.model.step3['type_3_advice'],
					'type_12_advice': this.model.step3['type_12_advice'],
					'type_60_advice': this.model.step3['type_60_advice'],
                    };
