...
export class AdvisorStep2Component implements OnInit {
	...
	private page_from = 1;
	step2_id : number;
	model = {type_3_diagnosis_id: '',type_12_diagnosis_id: '',type_60_diagnosis_id: '',step2:''};
	...
	private handleResponse(commonResponse:any) {
        if (commonResponse.success) {
            this.step2s = commonResponse.data.data;
            this.total = commonResponse.data.total_items;
            this.number_items =  this.step2s.length;
            this.paginate();
        } else {
            alert(commonResponse.error);
        }
	...
	editStep(step2_id){
		this.step2_id = step2_id;
        let data = {
            'step2_id': step2_id,
        };
        let url = 'advisor/step2/detail';
	...
	private handleResponseDetail(commonResponse:any) {
        if (commonResponse.success) {
			this.model.step2 = commonResponse.data[0];			
        } else {
            alert(commonResponse.error);
        }
    }
	...
	edit(){
		let url = 'advisor/step2/edit';
        let data = {
                    'step2_id' : this.step2_id,
                    'type_3_advice': this.model.step2['type_3_advice'],
					'type_12_advice': this.model.step2['type_12_advice'],
					'type_60_advice': this.model.step2['type_60_advice'],
                    };
