...
class StepThreeController extends Controller
{
	public function search(Request $request){
		...
		try{
			...
			if(empty($page_number) || empty($page_limit)){
				...
				$data = Step3::getListStep3($type_3_diagnosis_id,$type_12_diagnosis_id,$type_60_diagnosis_id,$page_limit,$page_number);
				$success = true;
				$error = null;
            }
	...
	public function detail(Request $request){
		$data = array();
		try{
			$step3_id = $request->input('step3_id');
			if(empty($step3_id)){
                $success = false;
                $error = \Lang::get('common_message.error.MISS_PARAM');
            }else{
				$data = Step3::getDetailStep3($step3_id);
				$success = true;
				$error = null;
            }
		}catch(\Illuminate\Database\QueryException $ex) {
	...
	public function edit(Request $request){
		...
		try{
			...
			if(empty($step3_id)){
				...
				$data = Step3::editStep3($step3_id,$type_3_advice,$type_12_advice,$type_60_advice);
				if($data){
					$success = true;
					$error = null;
				}else{
